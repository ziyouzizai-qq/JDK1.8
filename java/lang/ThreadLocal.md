...
1. <font color=red>Q：什么是弱引用？</font><br>
   A：java里面的引用分为强软弱虚四个引用强度，强引用，我们平时new的对象，没有引用指向才能被回收；软引用，当整个堆内存不够时，java虚拟机会尝试回收掉软引用的对象，如果回收了内存还不够，报OOM；弱引用：被弱引用指向的对象是熬不过第一次垃圾收集的；虚引用用的很少，主要作用与直接内存，被其引用的对象不能根据引用获取对象，这个对象回收后会收到一个通知。这些都是属于java虚拟机层面的，如果看的很吃力的话，我推荐周志明老师的深入了解JVM书籍，看过后会对JVM有所熟悉，对源码理解会更加透彻。<br>
2. <font color=red>Q：为什么Entry要继承弱引用？内存为什么泄露？</font><br>
   A：首先我来谈一下ThreadLocal的设计思路，假设我需要一个功能，有三个方法，A，B，C，他们的调用逻辑是A调B，B调C，现在我有一个需求，想将在A方法中获取的值用在C方法上，该如何做？估计很多人立刻会有答案，给B方法的参数列表上加参数，C方法列表上加参数，要知道ABC方法会有一个像水一样的线程，从A点流到C点，能否不影响BC点，拿这条“水”做文章呢？假设来个极端的设计，AC两点之间有若干个像B这样的方法存在呢。所以说ThreadLocal应运而生，ThreadLocal就像是一个入口，有点像时空穿梭一样，先A点扒一条口子，塞值进去，再从C点扒一条口子，取值进来。当然了，熟悉ThreadLocal的朋友知道，ThreadLocal并不只有这一个功能，还有一个强大的功能就是可以通过ThreadLocal来规避线程安全问题。但是ThreadLocal也逃避不了存值取值所带来的key的问题，例如：传统参数列表中所对应的引用就是key。所以设计ThreadLocal就会因为解决key的问题而引发内存泄露问题，当然了，内存泄露问题在你合理的使用ThreadLocal的情况下是可以规避的。
   现在来谈谈ThreadLocal的设计细节，将线程比喻成水，所以ThreadLocal肯定是拿“水”做文章的，也就是Thread这个类肯定会有一个容器来存放这个值的。在A点，将值存到Thread中，到C点，再从Thread中把值取出来，所以说ThreadLocal本质上并不是一个容器，容器是Thread，ThreadLocal仅仅只是操作Thread容器的一个方法入口，而且这个方法入口有个特征，就是AC两点都会通过这个方法入口来存取值，我为什么要强调这个特征呢，因为涉及到容器存取值的问题，其实大家都知道，不管是传统方法的引用，还是例如像HashMap这样，都会存在一个key用来存取值，而在操作Thread容器的时候又是以什么作为key呢。当然了，存取值也有一个特征，就是在AC两点存取值，这tm算什么特征？那这个特征和上述特征结合起来，ThreadLocal能否既作为操作Thread容器的方法入口，又作为存取值的key呢，因为这两个特征都出现在AC两点。所以ThreadLocal以自己为key，就省去了很多麻烦的操作。刚才我说过，java引用强度四种，其实在这里我们可以排除两个引用来设计ThreadLocal，一个是虚引用，哪怕对象不被回收，也不能访问对象，这肯定是不可以的，另一个是软引用，因为这种引用过于极端，内存够时，内存泄露(这里还没有讲为什么会内存泄露，读者可以回过头理解这句话)，内存不够时，被回收，那我这个程序员是希望内存够还是不够呢，程序员肯定希望内存够，够了就泄露，那我希望不够，不够不就触发垃圾收集，会将软引用给回收掉，那回收了内存还不够呢，OOM掉，所以不管够不够，两边都不行，因此也不在设计范围内。下面还剩下强引用和弱引用，强引用不用继承任何类，普通的类都是强引用。首先读者要知道一点，内存泄露是多个引用指向同一个对象，由于某个操作导致无用的对象或者是无法利用的对象被强引用指向而一直存活，从而得不到回收。我们通过ThreadLocal外部的实例引用（t1）设置值（如下代码），从下面代码可以知道ThreadLocal tl = new ThreadLocal();其t1作为外部引用，Entry.key作为内部引用，这两个引用指向同一个ThreadLocal实例，假设用强引用设计，一旦外部的引用tl = null;由于我取值是通过tl.get()去取的，但是我外部的key已经被我设置成null了，我不可能null.get()吧，因此我无法通过外部引用访问到当前线程的ThreadLocalMap的存储数据了，这样的数据就会以[key,value]的形式“胎死腹中”，此时会形成两个问题，第一，内存泄露了，第二，由于无用的数据和有用的数据都是以[key,value]的形式存在，无法判断哪个Entry是垃圾对象。那我们回过头来看，用弱引用能不能解决这两个问题。如果使用弱引用，t1 = null，同样以前存放的数据依旧无法取出来，但是由于内部引用key是弱引用，一旦外部强引用消失，在第一次垃圾收集就会被回收，所以以[null,value]存放在ThreadLocalMap中，相比于强引用的[key,value]形式，内存虽然还泄露，但是保证了key的内存是不泄露的。由于以[null,value]存放，对于造成内存泄露的对象我们就有了判断，只要key为null的肯定都是内存泄露对象，这样我们就可以根据key为null来清理value和node元素了，解析到这里，读者应该也知道为什么要用弱引用了吧。<br>
